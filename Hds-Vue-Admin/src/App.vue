...
<script>
  ...
  import utilsApi from './common/utils'
    ...
    methods: {
      ...
      refreshUserToken:function(){
         //从sessionStorage中取出当前用户
          let activeUser= utilsApi.getActiveUser();
          //取出cookie中的令牌
          let uid = utilsApi.getCookie("uid")
          ...
          if(activeUser && uid && uid == activeUser.uid){
              ...
              getjwt().then((res) => {
                  if(res.success){
                      let jwt = res.jwt;
                      let activeUser = utilsApi.getUserInfoFromJwt(jwt)
                      if(activeUser){
                          this.logined = true
                          this.user = activeUser;
                          utilsApi.setUserSession("activeUser",JSON.stringify(activeUser))
                      }
                  }
              })
          }
